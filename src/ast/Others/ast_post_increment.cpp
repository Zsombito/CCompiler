...
PostIncrement::PostIncrement(NodePtr variable)
{
    branches.push_back(variable);
}
void PostIncrement::VariableParse(Program_Data &program_data, std::string functionName)
{
    scopeNumber = program_data.functions[functionName].currentScope;
}
void PostIncrement::CodeGen(std::ostream &output, Program_Data &program_data, int destReg, std::string type) const
{
    ...
    if(variable_offset != -9)
    {
        ...
        output << "addiu $" << freeReg << ", $" << destReg << ", 1" << std::endl;
    ...
    else
    {
        ...
        output << "add $" << freeReg << ", $" << destReg << ", $" << freeReg << std::endl;
