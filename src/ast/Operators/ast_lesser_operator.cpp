...
LesserOperator::LesserOperator(NodePtr left, NodePtr right)
{
    branches.push_back(left);
    branches.push_back(right);
}
void LesserOperator::CodeGen(std::ostream &output, Program_Data &program_data, int destReg, std::string type) const 
{
    ...
    if(realType != "float")
    {
        ...
        output << "slt $" << destReg << ", $" << leftReg << ", $" << rightReg << std::endl;
    ...
    else
    {
        ...
        output << "c.lt.s"  << " $f" << leftReg << ", $f" << rightReg << std::endl;
