...
BOrOperator::BOrOperator(NodePtr left, NodePtr right)
{
    branches.push_back(left);
    branches.push_back(right);
}
void BOrOperator::CodeGen(std::ostream &output, Program_Data &program_data, int destReg, std::string type) const 
{
    ...
    output << "or $" << destReg << ", $" << leftReg << ", $" << rightReg << std::endl;
